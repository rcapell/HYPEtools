...
#' @importFrom ggplot2 scale_y_continuous theme theme_void unit waiver xlab ylab scale_x_log10 scale_y_log10
...
PlotPerformanceByAttribute <- function(subass, subass.column = 2, groups = NULL, attributes, join.type = c("join", "cbind"), group.join.type = c("join", "cbind"), groups.color.pal = NULL, drop = TRUE, alpha = 0.4,
  ...
  for (col in plotcols) {
    ...
    if (!is.null(groups)) {
      ...
      # Determine which groups get trendlines
      if (trendline == TRUE) {
        trendline_levels <- plotdata %>%
          group_by(.data[["Group"]]) %>%
    ...
    if (is.null(groups)) { # No groups
      ...
      table.p <- ggtexttable(table,
        ...
        theme = ttheme(
          ...
          tbody.style = tbody_style(size = 8, color = "black", fill = group_colors[as.character(table$Group)])
        )
      )
