...
# INTERNAL.hype.vars.info <- full_join(tables[[1]]%>%filter(!is.na(.[,"#"])), # Simulated Variables, Remove Rows with NA Number
...
#
# # Add Variable Name
# INTERNAL.hype.vars.info$Name<- unlist(lapply(INTERNAL.hype.vars.info$Description,function(X){desc(X)}))
#
# # Format Units
# INTERNAL.hype.vars.info[which(grepl("\\p{So}C",INTERNAL.hype.vars.info$Unit,perl=T)),"Unit"] <- "C" # Remove Degree Symbol from Temperatures
# INTERNAL.hype.vars.info[which(INTERNAL.hype.vars.info$Unit=="106m3"),"Unit"] <- "10^6 m3" # Fix Volume Units
# INTERNAL.hype.vars.info$Unit <- gsub("[\xb5]","micro ",INTERNAL.hype.vars.info$Unit) # Replace Mu (Hex Code &#xb5) with word Micro
#
#
#
# # _____________________________________________________________________________________________________________________________________
...
# fix_rows <- which(grepl(",",INTERNAL.hype.vars.info$ID))
#
# for(row in rev(fix_rows)){
#   # Get Variable Names
#   row_vars <- strsplit(INTERNAL.hype.vars.info$ID[row],",")[[1]]
#
#   # Add Rows for each Variable
#   for(var_name in rev(row_vars)){
#     INTERNAL.hype.vars.info <- INTERNAL.hype.vars.info%>%
#       add_row(INTERNAL.hype.vars.info[row,]%>%mutate("ID"=var_name),
#               .after=row)
#   }
#
#   # Remove Original Row
#   INTERNAL.hype.vars.info <- INTERNAL.hype.vars.info%>%slice(-row)
#
# }
#
# # Save Data to package sysdata
# load("R/sysdata.rda") # Load existing data in R/sysdata.rda
# save(INTERNAL.hype.vars.info,INTERNAL.hype.vars, file = "R/sysdata.rda")
