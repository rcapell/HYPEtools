...
```{r readGIS}
# Read GIS Files
map.Qobs <- st_read(file.path(model_path,"gis", "Nytorp_station.shp"))
map.subid <- st_read(file.path(model_path, "gis","Nytorp_map.shp"))

# Add SUBID to gauge station attributes
names(map.Qobs)
map.Qobs$SUBID <- 3587
...
#               sites = map.Qobs, sites.subid.column = 4, bg = map.subid,
#               plot.scale = F, plot.arrow = F, plot.label=T, bg.label.column = 25, plot.bg.label="static")
...
PlotMapOutput(mcrun, map = map.subid, map.subid.column = 21, var.name = "CRUN", legend.title = "Runoff (mm/d)", 
              col = ColQ, col.breaks = NULL, legend.pos = "right", map.adj = 0, plot.scale = F, plot.arrow = F)
...
#               col = ColQ, col.breaks = NULL, legend.pos = "bottomright", map.adj = 0, plot.scale = F, plot.arrow = F)
...
#               col = ColQ, col.breaks = NULL, legend.pos = "bottomright", map.adj = 0, plot.scale = T, plot.arrow = F, plot.label = T, plot.searchbar = T)
# 
