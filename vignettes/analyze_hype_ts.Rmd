...
# Plot to default device - select variables
# PlotBasinOutput(bas, gd = gd, hype.vars = c("cout", "rout"))

# Plot to default device - time series in R with a log scale for the flow (log.q = TRUE)
# PlotBasinOutput(bas, timestep = attr(bas, "timestep"),
#   hype.vars = "all", vol.err = TRUE, log.q = TRUE, start.mon = 10, from = 1,
...
```{r PlotBasinSummary}
# PlotBasinSummary(bas, gd = gd, gcl = gcl)
```
